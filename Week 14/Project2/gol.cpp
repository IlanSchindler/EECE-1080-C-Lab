/*
 * Project Title:
 * - The Game of Life (Now with Adjustable Rules)
 * Description: (should be a few sentences)
 * This program imports a text file that allows the user to dictate the cells which are alive at the start of the game, how many neighbors a
 * living cell needs to avoid dying of underpopulation, how many neighbors a dead cell needs to be born, and how much time the program
 * should wait before creating the next generation. The program explains the rules and needed format of the file to the user, imports the file,
 * then uses the rules and starting grid to play Conway's The Game of Life as the user dictated.
 *
 * Developers:
 * - Ilan Schindler - schindim@mail.uc.edu
 * - Steven LeVasseur - levasssk@mail.uc.edu
 *
 * Help Received:
 * - None
 *
 * Special Instructions: // optional
 * - Upon running, the program will explain in detail how to set up the txt file, so that the program can run it.
 * If the txt file does not comply with these specifications, a relevant error message will be displayed.
 * - An Excel file which greatly simplifies the file creation process has also been included but is not necessary for the program to run correctly.
 *
 * Developer comments: // Not optional
 * Developer 1 (Ilan Schindler):
 *      ---Please describe what tasks you performed and what your learned from your experience
 * I implemented the functions not listed in Steve's comments. I made the excel sheet and did the testing. I learned how to use the
 * SetConsoleCursorPosition function to update specific locations on the output console screen instead of completely reprinting the entire screen.
 *
 * Developer 2 (Steven LeVasseur):
 *      ---Please describe what tasks you performed and what your learned from your experience
 * I created the functions 'string start()', 'void cell::set', 'void cell::update', and 'void verifyins'; researched some of the commands used in the
 * program but not taught in class, such as 'Sleep()' and cursor visibility; created basic comments as the code was being written; and reformatted
 * the end-code for ease of readability while re-checking comments. I learned about the Sleep command, how to make the cursor invisible, and that
 * coding with 2 people is still strangely difficult, but doable.
 */
#include <iostream>
#include <windows.h>
#include <fstream>
...
#define ifnt else // This is a joke that Ilan put in because I showed him Reddit
...
void verifyins(int death, int life, int times){
    ...
    // note, at this point in tidying up the code and comments, Steve noticed that programming was misspelled but chose to leave it
 ...
 * or any values in the initial grid are not 0 or 1
...
// I do not believe it is used in the final program, but it was used during testing, so to be safe it shall remain
// note, Steve recognizes this as a joke from r/ProgrammerHumor. Ilan is not original
...
// note, Steve would like to point out that Ilan is not OCD enough to fix his spelling and grammar. That's my job for some reason
